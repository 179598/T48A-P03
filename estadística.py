...
# Generar estadísticas básicas sin usar pandas

# Cálculo de estadísticas para Altura
media_altura = np.mean(datos_mexico[:, 0])
desviacion_altura = np.std(datos_mexico[:, 0])
mediana_altura = np.median(datos_mexico[:, 0])
varianza_altura = np.var(datos_mexico[:, 0])
percentiles_altura = np.percentile(datos_mexico[:, 0], [25, 50, 75])

# Cálculo de estadísticas para Cintura
media_cintura = np.mean(datos_mexico[:, 1])
desviacion_cintura = np.std(datos_mexico[:, 1])
mediana_cintura = np.median(datos_mexico[:, 1])
varianza_cintura = np.var(datos_mexico[:, 1])
percentiles_cintura = np.percentile(datos_mexico[:, 1], [25, 50, 75])

# Mostrar estadísticas
print("Estadísticas de Altura:")
print(f"Media: {media_altura:.2f} m")
print(f"Desviación estándar: {desviacion_altura:.2f}")
print(f"Mediana: {mediana_altura:.2f}")
print(f"Varianza: {varianza_altura:.2f}")
print(f"Percentiles (25, 50, 75): {percentiles_altura}")

print("\nEstadísticas de Cintura:")
print(f"Media: {media_cintura:.2f} cm")
print(f"Desviación estándar: {desviacion_cintura:.2f}")
print(f"Mediana: {mediana_cintura:.2f}")
print(f"Varianza: {varianza_cintura:.2f}")
print(f"Percentiles (25, 50, 75): {percentiles_cintura}")

# Graficar histogramas
plt.hist(datos_mexico[:, 0], bins=10, edgecolor='black', alpha=0.7, color='blue')
plt.xlabel("Altura (m)")
plt.ylabel("Frecuencia")
...
plt.hist(datos_mexico[:, 1], bins=10, edgecolor='black', alpha=0.7, color='green')
